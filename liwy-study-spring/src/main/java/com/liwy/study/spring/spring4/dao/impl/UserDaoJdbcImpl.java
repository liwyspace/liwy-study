...
//        Long id = (Long) simpleJdbcInsert.executeAndReturnKey(param);
    ...
    public void simpleQueryChannelTag() {
        SimpleJdbcCall simpleJdbcCall = new SimpleJdbcCall(jdbcTemplate).withProcedureName("procQueryChannelTag")
                .returningResultSet("channels", BeanPropertyRowMapper.newInstance(Channel.class))
                .returningResultSet("tags", BeanPropertyRowMapper.newInstance(Tag.class));
        Map<String, Object> execute = simpleJdbcCall.execute();
        System.out.println(execute.get("channels"));
        System.out.println(execute.get("tags"));
    }
